...
bool framebuffer_t::remove(const UID& uid) {
	...
	if(it == std::end(m_Attachments.value))
		return false;
...
void framebuffer_t::set(core::resource::handle<core::gfx::sampler_t> sampler) {
	m_Sampler.value = sampler;
}


uint32_t framebuffer_t::framebuffers() const {
...
const UID& framebuffer_t::attachment::texture() const {
	return m_Texture.value;
}

const core::gfx::clear_value& framebuffer_t::attachment::clear_value() const {
	return m_ClearValue.value;
}
bool framebuffer_t::attachment::shared() const {
	return m_Shared.value;
}

framebuffer_t::attachment::operator core::gfx::attachment() const noexcept {
