...
grid::grid(state& state, entity target, resource::cache& cache, resource::handle<context> context,
		   ...
		   resource::handle<pipeline_cache> pipeline_cache, resource::handle<shader_buffer_binding> instanceMaterialBuffer,
	...
	: m_Target(target), m_Scale(scale), m_Offset(offset)
{
	...
	boxData2->transform(core::data::geometry::constants::POSITION, [](psl::vec3& pos) { pos -= psl::vec3::one * 0.5f; });
	...
	auto material1 = cache.create<core::gfx::material>(context, matData, pipeline_cache, instanceMaterialBuffer->buffer);
	...
	auto material2 = cache.create<core::gfx::material>(context, matData2, pipeline_cache, instanceMaterialBuffer->buffer);
	...
	// create entity
	m_Entities = state.create(2, psl::array<renderable>{ {m_Bundle, m_Geometry}, {bundle1, geometry2} }, transform{}, empty<grid::tag>{}, empty<dynamic_tag>{});
...
{
	m_Bundle->set("color", psl::vec4{ float(std::rand() % 100) / 100.0f, 0.45f, 1.f, 1.f } *0.25f);
