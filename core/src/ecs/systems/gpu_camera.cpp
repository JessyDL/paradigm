...
gpu_camera::gpu_camera(psl::ecs::state& state, core::resource::handle<core::os::surface> surface, core::resource::handle<core::gfx::shader_buffer_binding> binding, core::gfx::graphics_backend backend)
	...
	psl::ecs::pack<const core::ecs::components::camera, const core::ecs::components::transform> cameras)
{
	...
	if(cameras.size() >= m_Max)
		throw std::runtime_error(fmt::format("cannot allocate more than {}, but {} was requested for this frame", m_Max, cameras.size()));
							   ...
							   const core::ecs::components::camera& camera)
{
	...
	m_Binding->buffer->commit({ core::gfx::commit_instruction{&fdata, m_Binding->segment, memory::range{index * m_Binding->region.alignment(), (index + 1) * m_Binding->region.alignment()}} });
}