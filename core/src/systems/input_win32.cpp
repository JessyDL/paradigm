			...
			case WM_INPUT:
			{
				...
				if(raw->header.dwType == RIM_TYPEMOUSE)
				{
					...
					switch(raw->data.mouse.usButtonFlags)
					{
						...
						input.m_HeldMouseButtons.erase(std::remove(std::begin(input.m_HeldMouseButtons), std::end(input.m_HeldMouseButtons), mousecode::BTN_1), std::end(input.m_HeldMouseButtons));
						break;
						...
						input.m_HeldMouseButtons.erase(std::remove(std::begin(input.m_HeldMouseButtons), std::end(input.m_HeldMouseButtons), mousecode::BTN_2), std::end(input.m_HeldMouseButtons));
						break;
						...
						input.m_HeldMouseButtons.erase(std::remove(std::begin(input.m_HeldMouseButtons), std::end(input.m_HeldMouseButtons), mousecode::BTN_3), std::end(input.m_HeldMouseButtons));
						break;
						...
						input.m_HeldMouseButtons.erase(std::remove(std::begin(input.m_HeldMouseButtons), std::end(input.m_HeldMouseButtons), mousecode::BTN_4), std::end(input.m_HeldMouseButtons));
						break;
						...
						input.m_HeldMouseButtons.erase(std::remove(std::begin(input.m_HeldMouseButtons), std::end(input.m_HeldMouseButtons), mousecode::BTN_5), std::end(input.m_HeldMouseButtons));
						break;
						case RI_MOUSE_WHEEL:
						scroll_delta delta;
				...
				{
					if(raw->data.keyboard.Flags == RI_KEY_MAKE || raw->data.keyboard.Flags == (RI_KEY_MAKE |
																							   RI_KEY_E0) || raw->data.keyboard.Flags == (RI_KEY_MAKE | RI_KEY_E1))
					{
					...
					if(raw->data.keyboard.Flags == RI_KEY_BREAK || raw->data.keyboard.Flags == (RI_KEY_BREAK |
																								RI_KEY_E0) || raw->data.keyboard.Flags == (RI_KEY_BREAK | RI_KEY_E1))
					{
						surface->input().m_OnKeyReleased.Execute((keycode)WIN_NATIVE_TO_HID[raw->data.keyboard.VKey]);
						input.m_HeldKeys.erase(std::remove(std::begin(input.m_HeldKeys), std::end(input.m_HeldKeys), (keycode)WIN_NATIVE_TO_HID[raw->data.keyboard.VKey]), std::end(input.m_HeldKeys));
					}
				}
				delete[] lpb;
				return 0;
...
void input::tick()
{
	...
	for(const auto& key : m_HeldKeys)
		m_OnKeyHeld.Execute(key);
	for(const auto& key : m_HeldMouseButtons)
		m_OnMouseHeld.Execute(key);
}
#endif