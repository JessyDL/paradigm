...
namespace psl
{
	...
	class graph
	{
	  ...
	  public:
		...
		{
			auto it = std::find_if(std::begin(m_Edges), std::end(m_Edges), [&value](const auto& pair) {
		...
		{
			auto it = std::find_if(std::begin(m_Edges), std::end(m_Edges), [&source](const auto& pair) {
				return &pair.first->value == source;
			});
			if(it == std::end(m_Edges)) return false;

			auto target_it = std::find_if(std::begin(m_Edges), std::end(m_Edges), [&target](const auto& pair) {
				return &pair.first->value == target;
			});
					...
					if(node->references == 0 || visited[&node->value])
					{
						for(auto node_ref : list)
						{
							if((node_ref->references == 1 ||
								std::find(std::begin(result), std::end(result), &node_ref->value) ==
								  std::end(result)) &&
