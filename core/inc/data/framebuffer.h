...
namespace core::data
{
	...
	public:
		...
		///
		/// all data contained within this object is not guaranteed to be loaded, this includes the UID of the core::gfx::texture.
		...
		public:
			...
			private:
				...
				serialization::property<vk::SampleCountFlagBits, const_str("SAMPLE_COUNT_FLAGS", 18)> m_SampleCountFlags;
				serialization::property<vk::AttachmentLoadOp, const_str("LOAD_OP", 7)> m_LoadOp;
				serialization::property<vk::AttachmentStoreOp, const_str("STORE_OP", 8)> m_StoreOp;
				serialization::property<vk::AttachmentLoadOp, const_str("STENCIl_LOAD_OP", 15)> m_StencilLoadOp;
				serialization::property<vk::AttachmentStoreOp, const_str("STENCIl_STORE_OP", 16)> m_StencilStoreOp;
				serialization::property<vk::ImageLayout, const_str("INITIAL_LAYOUT", 14)> m_InitialLayout;
				serialization::property<vk::ImageLayout, const_str("FINAL_LAYOUT", 12)> m_FinalLayout;
			...
			/// \param[in] texture a UID pointing to a valid (known or constructed), core::gfx::texture object.
			...
			attachment(const UID& texture, const vk::ClearValue& clear_col, vk::AttachmentDescription descr, bool shared = false);

			/// \brief returns the UID assigned to this render attachment.
		...
		private:
			...
			serialization::property<UID, const_str("TEXTURE UID", 11)> m_Texture;
			serialization::property<vk::ClearValue, const_str("CLEAR VALUE", 11)> m_ClearValue;
			serialization::property<description, const_str("DESCRIPTION", 11)> m_Description;
			serialization::property<bool, const_str("SHARED", 6)> m_Shared;
		};
		...
		/// \param[in] uid the resouce system assigned UID.
		...
		framebuffer(const UID& uid, core::resource::cache& cache, uint32_t width, uint32_t height, uint32_t layers = 1u);
		framebuffer(const framebuffer& other, const UID& uid, core::resource::cache& cache);
		...
		const UID& add(uint32_t width, uint32_t height, uint32_t layerCount, vk::Format format, vk::ImageUsageFlags usage, vk::ClearValue clearValue, vk::AttachmentDescription descr);

		/// \brief removes the attachment that is using this UID for its texture.
		...
		/// \note the sampler does not need to be loaded, we will only store the UID, but if the framebuffer is used for rendering, and
		...
		std::optional<UID> sampler() const;
	...
	private:
		...
		serialization::property<std::vector<attachment>, const_str("ATTACHMENTS", 11)> m_Attachments;
		serialization::property<UID, const_str("SAMPLER", 7)> m_Sampler;
		serialization::property<uint32_t, const_str("WIDTH", 5)> m_Width;
		serialization::property<uint32_t, const_str("HEIGHT", 6)> m_Height;
		serialization::property<uint32_t, const_str("FRAMEBUFFER COUNT", 17)> m_Count;
