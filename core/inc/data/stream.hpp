...
namespace core
{
	namespace details
	{
		enum class memory_stream_type_e
		{
			single = 0,
			vec2   = 1,
			vec3   = 2,
		...
		struct memory_stream_underlying_from_type
		{
			...
			static constexpr memory_stream_type_e value = std::is_same_v<T, float>		 ? memory_stream_type_e::single
														  ...
														  : std::is_same_v<T, psl::mat3x3>
															? memory_stream_type_e::mat3x3
				...
				std::conditional_t<
				  ...
				  std::conditional_t<
					...
					std::conditional_t<
					  ...
					  psl::mat2x2,
					  std::conditional_t<
						Type == memory_stream_type_e::mat3x3,
						psl::mat3x3,
	...
	class vertex_stream_t
	{
	  ...
	  public:
		...
		{
			return std::visit([]<typename Y>(const Y& value) { return std::is_same_v<T, typename Y::unit_t>; },
							  m_Stream);
		}
