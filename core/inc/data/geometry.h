...
namespace core::data
{
	...
	///
	/// Unlike most engines, geometry data in paradigm is considered a collection of undescribed data, that has been keyed.
	/// There are some globally accepted keys defined for convenience at the core::data::geometry::constants namespace.
	/// Only 2 streams are mandatory (core::data::geometry::constants::POSITION and the indices stream), if those are not present, the data is considered invalid.
	...
	public:
		...
		std::optional<std::reference_wrapper<const core::stream>> vertices(const psl::string_view name = constants::POSITION) const;
