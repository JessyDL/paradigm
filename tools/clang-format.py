        ...
        for r, d, f in os.walk(folder):
            for file in f:
                if re.search(".*?\.(cpp|hpp|h)", file):
                    run_command([cformat, "-i", "-style=file", os.path.join(r, file)], print_stdout=True)

if __name__ == "__main__":
    parser = ArgumentParser()
    parser.add_argument("--directory", nargs='+', default=[os.path.join(os.path.dirname(os.path.realpath(__file__)), '..', 'core'), os.path.join(os.path.dirname(os.path.realpath(__file__)), '..', 'psl'), os.path.join(
        os.path.dirname(os.path.realpath(__file__)), '..', 'tests'), os.path.join(os.path.dirname(os.path.realpath(__file__)), '..', 'benchmarks')], help="Set the directories you wish to format recursively")
    parser.add_argument("--clang-format", type=str, default=None,
