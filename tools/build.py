...
class Paradigm(object):
    ...
    def initialize(
        ...
        build_arguments = ArgumentParser(
            description="Generate build files for the current project."
        )
        build_arguments.add_argument(
            ...
            default="auto",
            nargs="?",
            help="Set the generator for the project",
            dest="generator",
        )
        build_arguments.add_argument(
            "--arch",
            ...
            default="x64",
            nargs="?",
            help="Architecture type to generate for",
            dest="architecture",
        )
        build_arguments.add_argument(
            "--clean",
            ...
            default="",
            nargs="?",
            help="Allows you to clean the project files, build files, or all",
            dest="clean",
        )
        build_arguments.add_argument(
            "--root",
            ...
            default=directory,
            nargs="?",
            help="Override for the root directory to work from",
            dest="root_dir",
        )
        build_arguments.add_argument(
            "--build_dir",
            ...
            default="builds",
            nargs="?",
            help="Override for the build directory, this is relative to the root",
            dest="build_dir",
        )
        build_arguments.add_argument(
            "--project_dir",
        ...
        build_arguments.add_argument(
            ...
            default="Release",
            nargs="?",
            help="build configuration to use when the build flag is true",
            dest="build_config",
        )
        build_arguments.add_argument(
            "--vk_static",
            action="store_true",
            dest="vk_static",
            help="when this flag is set, vulkan will statically bind",
        )
        build_arguments.add_argument(
            "-u", "--update", action="store_true", dest="cmake_update"
        )
        build_arguments.add_argument(
        ...
        build_arguments.add_argument(
            ...
            "--build",
            action="store_true",
            help="build the target as well",
            dest="build",
        )
        build_arguments.add_argument(
            "--cmake_params", action="store", type=str, nargs="*", dest="cmake_params"
        )
        build_arguments.add_argument(
            "--graphics",
            action="store",
    ...
    def prepare(self, args):
        ...
        args.project_dir = os.path.join(
            args.root_dir, args.project_dir, args.generator, args.architecture
        )
        args.build_dir = os.path.join(
            args.root_dir, args.build_dir, args.generator, args.architecture
        )
    ...
    def generate_command(self, args):
        ...
        try:
            ...
            print(
                "Missing dependency, please install cmake and make it visible in the $PATH variable of your terminal"
            )
            raise
        ...
        if (
            not os.path.exists(os.path.join(args.project_dir, "CMakeFiles"))
            or not self.has_generated
        ):
