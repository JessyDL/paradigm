...
class Android:
    def __init__(
        self, directory, sdk=None, gradle=None, bundletool=None, skip_setup=False
    ) -> None:
        self._sdk = sdk
        ...
        self._generated = os.path.exists(directory) and os.path.exists(
    ...
    def generate(self, overwrite=False) -> None:
        ...
        if not os.path.exists(os.path.join(self._directory, "settings.gradle")):
            ...
            shutil.copyfile(
                os.path.join(android_build_root, "gradle.properties"),
                os.path.join(self._directory, "gradle.properties"),
            )
        ...
        [
            os.symlink(os.path.join(ROOT_DIR, dir), os.path.join(cpp_dir, dir))
            for dir in ["psl", "core", "extern", "cmake", "CMakeLists.txt", "tools"]
            if not os.path.exists(os.path.join(cpp_dir, dir))
        ]
    ...
    def build(self, type="debug", arguments=None) -> None:
        ...
        if arguments is not None:
            commands.extend(
                [
                    f"-Pparadigm.{key}={value}"
                    for key, value in zip(*[iter(arguments)] * 2)
                ]
            )

        commands.append("--stacktrace")
        run_command(
            ["./gradlew"] + commands, directory=self._directory, print_stdout=True
        )
        self._regenerate_apks.add(type)

    def build_apks(self, type="debug") -> None:
        if not os.path.exists(
            os.path.join("main", "build", "outputs", "bundle", type, f"main-{type}.aab")
        ):
    ...
    def install(self, type="debug"):
        if not os.path.exists(
            os.path.join(
                "main", "build", "outputs", "bundle", type, f"main-{type}.apks"
            )
        ):
        ...
        run_command(
            [
                application,
                "install-apks",
                f"--apks={os.path.join('main', 'build', 'outputs', 'bundle', type, f'main-{type}.apks')}",
            ],
            directory=self._directory,
            print_stdout=True,
        )
    ...
    def run(self):
        output = run_command(
            ["adb" "shell" "pm" "list" "packages", "com.paradigmengine.main"],
            catch_stdout=True,
        )
        ...
        application = (
            os.path.join(self._sdk, "platform-tools", "adb")
            if self._sdk != None
            else "adb"
        )
        run_command(
            [
                ...
                "start",
                "-n",
                "com.paradigmengine.main/com.paradigmengine.main.MainActivity",
            ],
            print_stdout=True,
        )
...
def main():
    ...
    parser.add_argument(
        "--output",
        type=str,
        help="output directory for the build, the provided cmake script typically sets this to builds/android/{arch}",
    )
    parser.add_argument(
        "--sdk",
        default=None,
        help="set to override the used android sdk, falls back to path available one",
    )
    parser.add_argument(
        "--gradle",
        default=None,
        help="set to override the used gradle, falls back to path available one",
    )
    parser.add_argument(
        "--bundletool",
        default=None,
        help="set to override the used bundletool, falls back to path available one",
    )
    parser.add_argument(
        "-f",
        "--force",
        action="store_true",
        help="Forcibly generate the build directory even if it already exists",
    )
    parser.add_argument(
        "-r", "--run", action="store_true", help="run the intent after installation"
    )
    ...
    parser.add_argument(
        "--purge",
        action="store_true",
        help="Delete previous install, and generate from scratch",
    )
    parser.add_argument(
        "--skip-setup",
        action="store_true",
        help="Skip the setup phase where the SDK tools are installed",
    )
