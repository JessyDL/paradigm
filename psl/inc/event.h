...
namespace common
{
	...
	protected:
		struct internal_event_t
		{
			...
			//internal_event_t(internals::event::event_signature_t signature, functype func) : signature(signature), function(func) {};
			internal_event_t(const internals::event::event_signature_t &signature, functype &&func) : signature(signature), function(std::forward<functype>(func)) {};
	...
	protected:
		...
		private:
			...
			{
				m_Listeners.emplace_back(std::move(internals::event::event_signature_t(func)), std::move(std::bind(func, internals::event::placeholder<indices + 1>::ph...)));
			}
			...
			{
				m_Listeners.emplace_back(std::move(internals::event::event_signature_t(mThis, func)), std::move(std::bind(func, mThis, internals::event::placeholder<indices + 1>::ph...)));
			}
	};
	...
	public:
		...
		void Subscribe(Class *mThis, Function&& func)
		{
			m_Listeners.emplace_back(std::move(internals::event::event_signature_t(mThis, func)), std::move(std::bind(func, mThis)));
		}
...
{
	template<int N>
	struct is_placeholder< internals::event::placeholder<N> >
