...
namespace psl
{
	...
	class view_ptr
	{
	  public:
		...
		~view_ptr()						= default;
		view_ptr(const view_ptr& other) = default;
		view_ptr(view_ptr&& other)		= default;
		view_ptr& operator=(const view_ptr& other) = default;
		view_ptr& operator=(view_ptr&& other) = default;

		constexpr pointer get() const noexcept { return m_Value; }

		constexpr reference operator*() const { return assert(m_Value != nullptr), *m_Value; }
