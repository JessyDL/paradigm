	...
	namespace barriers
	{
		class memory
		{
		  private:
			...
			{
				(..., void(m_Barriers[index][S] = ::memory::range((std::uintptr_t)&std::get<S>(barrier),
																  (std::uintptr_t)&std::get<S>(barrier) +
																	  sizeof(decltype(std::get<S>(barrier))))));
				//(..., void(m_Barriers[index][S].end = &std::get<S>(dependencies) + sizeof(decltype(std::get<S>()))));
			}

		  public:
			memory(const ::memory::range& range, barrier barrier) { add(range, barrier); }

			/*template<typename R, typename ... Args>
			memory(const psl::evoke<R, Args...>& evocable)
			{
				auto type_qualifiers = utility::templates::get_type_qualifier_v<typename psl::evoke<R,
			Args...>::tuple_type>{}.value; auto type_locations = evocable._parameter_locations();
				...
				for (auto i = 0ui64; i < sizeof...(Args); ++i)
				{
					...
					if (type_qualifier == utility::templates::type_qualifier::IS_POINTER || type_qualifier ==
			utility::templates::type_qualifier::IS_CONST_POINTER)
					{
						...
						barrier dep = (type_qualifier == utility::templates::type_qualifier::IS_CONST_POINTER) ?
			barrier::READ : barrier::WRITE; add(::memory::range{ type_location.first, type_location.first +
			sizeof(void*) }, dep); add(::memory::range{ (std::uintptr_t)objLoc, (std::uintptr_t)objLoc +
			type_location.second }, dep);
					}
					else if (type_qualifier == utility::templates::type_qualifier::IS_VALUE_REFERENCE || type_qualifier
			== utility::templates::type_qualifier::IS_CONST_VALUE_REFERENCE)
					{
						barrier dep = (type_qualifier == utility::templates::type_qualifier::IS_CONST_VALUE_REFERENCE) ?
			barrier::READ : barrier::WRITE; add(::memory::range{ type_location.first, type_location.first +
			type_location.second }, dep);
					}
				}
...
*/
			...
			memory(Range_Iterator&& range_begin, Range_Iterator&& range_end, Barrier_Iterator&& barrier_begin,
				   Barrier_Iterator&& barrier_end)
			{
				add(range_begin, range_end, barrier_begin, barrier_end);
			}

			memory(std::vector<::memory::range>&& read_barriers  = {},
				   std::vector<::memory::range>&& write_barriers = {})
				: m_Barriers{std::forward<std::vector<::memory::range>>(read_barriers),
							 std::forward<std::vector<::memory::range>>(write_barriers)}
			{
			...
			memory& operator+=(const memory& other)
			{
				if(this != &other)
				{
					for(size_t count = 0; count < m_Barriers.size(); ++count)
					{
						...
						std::copy(std::begin(other.m_Barriers[count]), std::end(other.m_Barriers[count]),
								  std::begin(m_Barriers[count]) + size);
						std::inplace_merge(std::begin(m_Barriers[count]), std::begin(m_Barriers[count]) + size,
										   std::end(m_Barriers[count]));
			...
			template <typename Range_Iterator, typename Barrier_Iterator>
			void add(Range_Iterator&& range_begin, Range_Iterator&& range_end, Barrier_Iterator&& barrier_begin,
					 Barrier_Iterator&& barrier_end)
			{
				...
				while(rangeIt != range_end && depIt != barrier_end)
				{
					...
					m_Barriers[dep].insert(
						std::upper_bound(std::begin(m_Barriers[dep]), std::end(m_Barriers[dep]), *rangeIt), *rangeIt);
			...
			template <typename Range_Iterator, typename Barrier_Iterator>
			void remove(Range_Iterator&& range_begin, Range_Iterator&& range_end, Barrier_Iterator&& barrier_begin,
						Barrier_Iterator&& barrier_end);
		...
		class task
		{
		  public:
			...
			task(details::task_token* owner, std::vector<details::task_token*>&& task_barriers)
				: m_Task(owner), m_TaskBarrier(std::forward<std::vector<details::task_token*>>(task_barriers)){};
	...
	namespace details
	{
		...
		{
		  public:
			task_token(scheduler& scheduler, std::vector<details::task_token*>&& task_barriers = {},
					   barriers::memory&& memory_barriers = {})
				: m_Scheduler(scheduler),
				  m_TaskBarrier(this, std::forward<std::vector<details::task_token*>>(task_barriers)),
				  m_MemBarrier(std::forward<barriers::memory>(memory_barriers)){};
			...
			template <typename R, typename... Args>
			std::shared_ptr<task<R, Args...>> then(R (*function)(Args...), Args&&... args,
												   std::vector<details::task_token*>&& task_barriers = {},
												   barriers::memory&& memory_barriers				 = {});
	...
	class task final : public details::task_token
	{
	  ...
	  public:
		task(scheduler& scheduler, std::vector<details::task_token*>&& task_barriers,
			 barriers::memory&& memory_barriers, R (*function)(Args...), Args&&... args)
			: details::task_token(scheduler, std::forward<std::vector<details::task_token*>>(task_barriers),
								  std::forward<barriers::memory>(memory_barriers)),
			  m_Params(std::forward<Args>(args)...), m_Func(function)
		{
			add_memory_dependency(m_Params);
		};

		task(scheduler& scheduler, std::vector<details::task_token*>&& task_barriers, R (*function)(Args...),
			 Args&&... args)
			: details::task_token(scheduler, std::forward<std::vector<details::task_token*>>(task_barriers)),
			  m_Params(std::forward<Args>(args)...), m_Func(function)
		{
		...
		task(scheduler& scheduler, R (*function)(Args...), Args&&... args)
			: details::task_token(scheduler), m_Params(std::forward<Args>(args)...), m_Func(function)
		{
			add_memory_dependency(m_Params);
		};

		R& value() { return m_Result; }

		const R& cvalue() const { return m_Result; }

	  private:
		void execute_impl() override { m_Result = execute_fn(std::index_sequence_for<Args...>{}); }
	...
	class task<void, Args...> final : public details::task_token
	{
	  ...
	  public:
		task(scheduler& scheduler, void (*function)(Args...), Args&&... args,
			 std::vector<details::task_token*>&& task_barriers = {}, barriers::memory&& memory_barriers = {})
			: details::task_token(scheduler, std::forward<std::vector<details::task_token*>>(task_barriers),
								  std::forward<barriers::memory>(memory_barriers)),
			  m_Params(std::forward<Args>(args)...), m_Func(function){};
	  ...
	  private:
		void execute_impl() override { execute_fn(std::index_sequence_for<Args...>{}); }
	...
	{
		return std::all_of(std::begin(tasks), std::end(tasks),
						   [](details::task_token* task) { return task->is_finished(); });
	}

	static bool is_any_finished(const std::vector<details::task_token*>& tasks)
	{
		return std::any_of(std::begin(tasks), std::end(tasks),
						   [](details::task_token* task) { return task->is_finished(); });
	}

	static size_t finished_count(const std::vector<details::task_token*>& tasks)
	{
		return std::count_if(std::begin(tasks), std::end(tasks),
							 [](details::task_token* task) { return task->is_finished(); });
	}
	...
	{
		for(auto& token : tasks) token->wait_until_finished();
	}
	...
	class scheduler final
	{
		...
		class base_unit
		{
		  public:
			...
			void start_thread() { m_Worker = std::thread(&base_unit::thread_loop, this); }
		...
		class job_divider final : public base_unit
		{
		  ...
		  public:
			...
			void manual_start() { on_start(); }

			void manual_awake() { worker_function(); }

			void manual_terminate() { on_terminate(); }
		  ...
		  private:
			...
			{
				for(auto& it : m_Workers) it->start_thread();
			}
	  ...
	  public:
		...
		void execute() { m_JobThread.awake(); }

		template <typename R, typename... Args>
		std::shared_ptr<task<R, Args...>> schedule(std::vector<details::task_token*>&& task_barriers,
												   barriers::memory&& memory_barriers, R (*function)(Args...),
												   Args&&... args)
		{
			// memory_barriers += { evocable };

			std::shared_ptr<details::task_token> task_handle{
				std::make_shared<task<R, Args...>>(*this, function, std::forward<Args>(args)...,
												   std::forward<std::vector<details::task_token*>>(task_barriers),
												   std::forward<barriers::memory>(memory_barriers))};
...
template <typename R, typename... Args>
inline std::shared_ptr<psl::async::task<R, Args...>>
psl::async::details::task_token::then(R (*function)(Args...), Args&&... args,
									  std::vector<details::task_token*>&& task_barriers,
									  barriers::memory&& memory_barriers)
{
	task_barriers.push_back(this);
	return m_Scheduler.schedule(std::forward(function), std::forward<Args>(args)...,
								std::forward<std::vector<details::task_token*>>(task_barriers),
								std::forward<barriers::memory>(memory_barriers));
	// return new psl::async::task<O, Args...>(m_Scheduler, std::forward<psl::evoke<O, Args...>>(evocable),
	// std::forward<std::vector<details::task_token*>>(task_barriers), std::forward<barriers::memory>(memory_barriers));
}
