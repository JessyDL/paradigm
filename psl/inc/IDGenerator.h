...
namespace psl
{
	...
	class generator
	{
	  private:
		...
		std::vector<range> m_Ranges{}; // Sorted array of ranges of free IDs
		T m_Max{0};
	  public:

		  explicit generator(T max)	: m_Max(max)
		  {
			  m_Ranges.emplace_back(0, m_Max);
		  }
		  ...
		  generator(generator &&other)
			  : m_Ranges(std::move(other.m_Ranges)), m_Max(other.m_Max)
		  {
		  }
		  ...
		  T capacity() const { return m_Max; }
		  bool create(T &id)
		  {
			  if(m_Ranges.size() == 0)
				  return false;
			  auto range_it = std::begin(m_Ranges);
			  if(range_it.first <= range_it.last)
			  {
				  id = range_it.first;
		  ...
		  std::pair<bool, T> create()
		  {
			  T id{};
			  auto res = create(id);
			  return std::make_pair(res, id);
		  }

		  bool create(T &id, T count)
		  {
			  for(auto range_it = std::begin(m_Ranges); range_it != std::end(m_Ranges); ++range_it)
			  {
				  const T range_count = range_it->size();
