...
namespace memory
{
	...
	/// and even allows constructor/destructors to be called (using the memory::region::create()/memory::region::destroy() methods).
	/// Memory regions do not allocate memory in one go, they reserve and "grow to" the specified size (like how virtual memory behaves).
	/// How they do this is platform specific however, but should not be of concern for the end-user.
	class region
	{
		...
		region(region& parent, memory::segment& segment, uint64_t pageSize, uint64_t alignment, allocator_base* allocator = new default_allocator());
	public:
		...
		[[nodiscard]] std::optional<memory::region> create_region(size_t size, std::optional<size_t> alignment, allocator_base* allocator = new default_allocator());
		...
		{
			memory::range temp_range{(std::uintptr_t)((void*)(&target)), (std::uintptr_t)((void*)(&target)) + sizeof(T)};
