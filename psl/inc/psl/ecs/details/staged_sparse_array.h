...
namespace psl::ecs::details
{
	...
	class staged_sparse_array
	{
	  ...
	  public:
		...
		constexpr bool has(index_t index) const noexcept
		{
			return has(index, 0, 1);
		}
	  ...
	  private:
		auto insert_impl(chunk_t& chunk, index_t offset, index_t user_index)
		{
			...
			std::memmove((T*)m_DenseData.data() + m_StageStart[2] + 1, (T*)m_DenseData.data() + m_StageStart[2], (m_Reverse.size() - m_StageStart[2]) * sizeof(T));
		...
		void grow()
		{
			...
			if(m_DenseData.size() < capacity * sizeof(T))
			{
				...
				assert_debug_break((m_Reverse.capacity() + 1) * sizeof(value_t) <= m_DenseData.size() && (m_Reverse.capacity() + 2) * sizeof(value_t) >= m_DenseData.size());
			}
		}
	...
	public:
		...
		bool operator[](index_t index) const noexcept
		{
			return has(index);
		}
