...
namespace psl::ecs::details
{
    ...
    static inline psl::array<entity>::iterator on_condition(
                                                ...
                                                psl::array<entity>::iterator end) noexcept
    {
        ...
        if constexpr(std::is_same_v<psl::ecs::execution::parallel_unsequenced_policy, psl::ecs::execution::no_exec>)
        {
            return std::remove_if(
                begin, end, [&state, &pred](entity lhs) -> bool { return !pred(state.get<T>(lhs)); });
        }
        else
        {
    ...
    static inline on_condition(
        const psl::ecs::state_t& state,psl::array<entity>::iterator begin, psl::array<entity>::iterator end, Pred&& pred) noexcept
    {
        if constexpr(std::is_same_v<psl::ecs::execution::parallel_unsequenced_policy, psl::ecs::execution::no_exec>)
        {
            return std::remove_if(
