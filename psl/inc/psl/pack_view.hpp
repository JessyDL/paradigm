...
namespace details
{
	...
	class unpack_iterator
	{
	  ...
	  public:
		unpack_iterator(const value_type& data) : data(std::get<typename psl::array_view<Ts>::iterator>(data)...) {};
...
class pack_view
{
  ...
  private:
	...
	iterator_begin(const range_t& t, size_t index, std::integer_sequence<T, indices...> indices_sequence) const noexcept
	{
	...
	template <typename T, T... indices>
	constexpr std::tuple<Ts*...>
	iterator_end(const range_t& t, size_t index, std::integer_sequence<T, indices...> indices_sequence) const noexcept
	{
  ...
  public:
	pack_view(psl::array_view<Ts>... views) : m_Pack(std::make_tuple(std::forward<psl::array_view<Ts>>(views)...))
	{
		...
		auto test = [](size_t size1, size_t size2) {
			psl_assert(size1 == size2 || size1 == 0, "expected size1 {} == size2 {} or size1 == 0 ", size1, size2);
		};
