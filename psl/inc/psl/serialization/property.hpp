...
#pragma region constructors
			...
			constexpr anonymous_property(Args&&... args) noexcept(
			...
			constexpr auto
			operator=(const anonymous_property& rhs) noexcept(std::is_nothrow_copy_assignable_v<value_type>)
...
#pragma region assignment
			...
			template <typename rhs_t>
			constexpr force_inline auto operator*=(const rhs_t& rhs) noexcept(
			  concepts::IsMultiplicationAssignableNothrow<
				value_type,
				rhs_t>) requires(concepts::IsMultiplicationAssignable<value_type, rhs_t>)
			{
				return value *= rhs;
			}
...
#pragma region incdec
			...
			constexpr force_inline auto operator++(int) noexcept(
			  concepts::IsIncrementablePostNothrow<value_type>) requires(concepts::IsIncrementablePost<value_type>)
			{
				return value++;
			}
			...
			constexpr force_inline auto operator--(int) noexcept(
			  concepts::IsDecrementablePostNothrow<value_type>) requires(concepts::IsDecrementablePost<value_type>)
			{
				return value--;
			}
#pragma endregion
#pragma region arithmetic
			...
			constexpr force_inline auto operator+(const rhs_t& rhs) const noexcept(
			  concepts::HasAdditionNothrow<value_type, rhs_t>) requires(concepts::HasAddition<value_type, rhs_t>)
			{
				return value + rhs;
			}
			...
			template <typename rhs_t>
			constexpr force_inline auto operator/(const rhs_t& rhs) const noexcept(
			  concepts::HasDivisionNothrow<value_type, rhs_t>) requires(concepts::HasDivision<value_type, rhs_t>)
			{
				return value / rhs;
			}
			...
			template <typename rhs_t>
			constexpr force_inline auto operator<<(const rhs_t& rhs) const
			  noexcept(concepts::HasBitLeftShiftNothrow<value_type, rhs_t>) requires(
				concepts::HasBitLeftShift<value_type, rhs_t>)
			{
				return value << rhs;
			}
...
#pragma region member_access
			...
			constexpr force_inline auto operator->() -> value_type* { return std::addressof(value); }
			constexpr force_inline auto operator->() const -> const value_type* { return std::addressof(value); }
