...
namespace utility::templates
{
	...
	enum class type_qualifier : uint8_t
	{
		...
		IS_CONST_POINTER = (uint8_t)type_qualifier_value::CONST_MOD | !(uint8_t)type_qualifier_value::VALUE | (uint8_t)type_qualifier_value::REFERENCE,
		IS_CONST_VALUE_REFERENCE = (uint8_t)type_qualifier_value::CONST_MOD | (uint8_t)type_qualifier_value::VALUE | (uint8_t)type_qualifier_value::REFERENCE,
		IS_CONST_VALUE = (uint8_t)type_qualifier_value::CONST_MOD | (uint8_t)type_qualifier_value::VALUE | !(uint8_t)type_qualifier_value::REFERENCE
	};
	...
	{
		static constexpr std::array<type_qualifier, sizeof...(T)> value{ utility::templates::get_type_qualifier<T>{}.value ...};
	};
...
namespace Common
{
	...
	public:
		...
		evoke(std::function<R(T...)>&& f, T... args) : bind_(std::forward<std::function<R(T...)>>(f)), params(std::forward<T>(args)...)
		{
		}
	...
	private:		
		...
		{
			std::array<std::pair<std::uintptr_t, size_t>, sizeof...(T)> result{ std::make_pair((std::uintptr_t)&std::get<S>(params), sizeof(T)) ... };
	...
	public:
		...
		evoke(std::function<R()>&& f)
			: bind_([f{ std::forward<std::function<R()> >(f) }](){	return (f)(); })
		{
		}
		~evoke() {};
	...
	public:
		...
		evoke(std::function<void()>&& f)
			: bind_([f{ std::forward<std::function<void()> >(f) }](){ (f)(); })
		{
		}
		~evoke() {};
