...
namespace psl
{
	...
	public:
		...
		buffer(std::initializer_list<T> values, memory::region* region) :
			m_Allocator(region->allocator()),
			m_Segment((values.size() * sizeof(T) < SBO_Size) ? std::nullopt : m_Allocator->allocate(sizeof(T) * values.size())),
			first((!m_Segment) ? (T*)m_Storage.storage() : (T*)m_Segment.value().range().begin),
			last(first),
		...
		buffer(const buffer& other) noexcept :
			...
			m_Segment((other.capacity() * sizeof(T) < SBO_Size) ? std::nullopt : m_Allocator->allocate(sizeof(T) * other.capacity())),
			first((!m_Segment) ? (T*)m_Storage.storage() : (T*)m_Segment.value().range().begin),
		...
		buffer(buffer&& other) noexcept :
			...
			m_Allocator(other.m_Allocator),
			m_Segment((other.capacity() * sizeof(T) < SBO_Size) ? std::nullopt : (other.using_sbo()) ? m_Allocator->allocate(sizeof(T) * other.capacity()) : other.m_Segment),
			first(
				(!m_Segment) ?
					(T*)m_Storage.storage() :
					(T*)m_Segment.value().range().begin),
			last(first + other.size()),
			m_Capacity(first + other.capacity())
		{
