...
formatted_string_buffer::formatted_string_buffer(const psl::string8_t& dividerOpen,
												 const psl::string8_t& dividerClose) :
	hasDivider(true),
	dividerOpen(dividerOpen), dividerClose(dividerClose)
{}
...
void formatted_string_buffer::IncreaseIndent()
{
	...
	{
		if(hasDivider) emplace_back(dividerOpen);
		m_IndentLevel += 1;
	}
	else
...
void formatted_string_buffer::DecreaseIndent()
{
	...
	{
		m_IndentLevel -= 1;
		if(hasDivider) emplace_back(dividerClose);
	}
	else
