...
bool range_t::contains(const range_t& other) const {
	return other.begin >= begin && other.end <= end;
}

bool range_t::is_contained_by(const range_t& other) const {
...
bool range_t::touches(const range_t& other) const {
	return other.begin == end || other.end == begin;
}

size_t range_t::size() const noexcept {
	return end - begin;
};
