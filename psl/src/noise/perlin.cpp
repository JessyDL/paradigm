...
void perlin::reseed(uint32_t seed) noexcept 
{
	...
	std::default_random_engine engine(seed);
	std::shuffle(std::begin(m_Permutation), std::end(m_Permutation), engine);
	std::copy(std::begin(m_Permutation), std::next(std::begin(m_Permutation), 256),
			  std::next(std::begin(m_Permutation), 256));
}
...
float perlin::noise(psl::vec3 coordinate, int32_t octaves, float persistence) const noexcept
{
	...
	for(int32_t i = 0; i < octaves; ++i)
	{
		...
		auto value = lerp(
			w,
			lerp(v,
				 lerp(u, grad(m_Permutation[AA], fcoordinate[0], fcoordinate[1], fcoordinate[2]),
					  grad(m_Permutation[BA], fcoordinate[0] - 1, fcoordinate[1], fcoordinate[2])),
				 lerp(u, grad(m_Permutation[AB], fcoordinate[0], fcoordinate[1] - 1, fcoordinate[2]),
					  grad(m_Permutation[BB], fcoordinate[0] - 1, fcoordinate[1] - 1, fcoordinate[2]))),
			lerp(v,
				 lerp(u, grad(m_Permutation[AA + size_t{1}], fcoordinate[0], fcoordinate[1], fcoordinate[2] - 1),
					  grad(m_Permutation[BA + size_t{1}], fcoordinate[0] - 1, fcoordinate[1], fcoordinate[2] - 1)),
				 lerp(u, grad(m_Permutation[AB + size_t{1}], fcoordinate[0], fcoordinate[1] - 1, fcoordinate[2] - 1),
...
float perlin::noise(psl::vec2 coordinate, int32_t octaves, float persistence) const noexcept
{
	...
	for(int32_t i = 0; i < octaves; ++i)
	{
		...
		auto value = lerp(
			w,
			lerp(v,
				 lerp(u, grad(m_Permutation[AA], fcoordinate[0], fcoordinate[1]),
					  grad(m_Permutation[BA], fcoordinate[0] - 1, fcoordinate[1])),
				 lerp(u, grad(m_Permutation[AB], fcoordinate[0], fcoordinate[1] - 1),
					  grad(m_Permutation[BB], fcoordinate[0] - 1, fcoordinate[1] - 1))),
			lerp(
				v,
				lerp(u, grad(m_Permutation[AA + size_t{1}], fcoordinate[0], fcoordinate[1]),
					 grad(m_Permutation[BA + size_t{1}], fcoordinate[0] - 1, fcoordinate[1])),
				lerp(u, grad(m_Permutation[AB + size_t{1}], fcoordinate[0], fcoordinate[1] - 1),
