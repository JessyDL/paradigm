...
{
	if (m_Type != type::REFERENCE_RANGE)
		return {};
	return *((reference_range_t*)(buffer));
}
...
{
	if (m_Type != type::COLLECTION)
		return {};
	return *((collection_t*)(&buffer));
}


size_t search(const psl::string8::view::const_iterator& first, const psl::string8::view::const_iterator& last, const psl::string8::view& pattern)
{
	//auto res = std::search(first, last, std::boyer_moore_searcher<psl::string8::view::const_iterator>(pattern.cbegin(), pattern.cend()));
#ifdef _MSC_VER
	auto res = std::search(first, last, std::default_searcher<psl::string8::view::const_iterator>(pattern.cbegin(), pattern.cend()));
#else
...
#endif
	//auto res = std::search(first, last, std::boyer_moore_horspool_searcher<psl::string8::view::const_iterator>(pattern.cbegin(), pattern.cend()));
	if (res == last)
...
std::tuple<type, size_t> parse_type(const psl::string8::view& content,  size_t node_content_begin_n)
{
	...
	return std::make_tuple(
		...
		((content.compare(content_begin_n + 1, constants::HEAD_OPEN.size(), constants::REFERENCE) == 0)	? type::REFERENCE_RANGE : type::VALUE_RANGE) :
		(content.compare(content_begin_n, constants::HEAD_OPEN.size(), constants::REFERENCE) == 0)		? type::REFERENCE : type::VALUE, content_begin_n);
}

bool is_literal(const psl::string8::view& content, size_t node_content_begin_non_empty_n)
{
	return content.compare(node_content_begin_non_empty_n, constants::LITERAL_OPEN.size(), constants::LITERAL_OPEN) == 0;
}


psl::string8_t::size_type rfind_first_not_of(
	psl::string8::view const& str,
	psl::string8_t::size_type const pos,
	psl::string8::view const& chars)
{
	if (chars.find(str[pos - 1]) != psl::string8_t::npos)
		return rfind_first_not_of(str, pos - 1, chars);
	return pos? pos:psl::string8_t::npos;
}

psl::string8_t::size_type rfind_first_of(
	psl::string8::view const& str,
	psl::string8_t::size_type const pos,
	psl::string8::view const& chars)
{
	if (pos > 0 && chars.find(str[pos - 1]) != psl::string8_t::npos)
		return  pos;
...
bool psl::format::container::remove(data & data)
{
	...
	case type::COLLECTION:
	{
		...
		while (current > index && content_shift_value ==0)
		{
			...
			case type::VALUE_RANGE:
			{
				...
				if (values.size() > 0)
				{
					content_shift_value = values[values.size() - 1].second.first + values[values.size() - 1].second.second - content_shift_index;					
				}
			}break;
			default: {};
			}
	...
	{
		m_Content.erase(m_Content.begin() + content_shift_index, m_Content.begin() + content_shift_index + content_shift_value);
	...
	{
		const auto& children_n = data.reinterpret_as_collection();
		name_shift_value = m_NodeData[index + children_n].m_Name.first + m_NodeData[index + children_n].m_Name.second - name_shift_index;
	}

	m_InternalData.erase(m_InternalData.begin() + name_shift_index, m_InternalData.begin() + name_shift_index + name_shift_value);
	...
	if (auto index = search(header, "CHECKSUM"); index != psl::string8_t::npos)
	{
		...
		if(features.verify_checksum)
		{
			...
			if(checksum != calc_checksum)
				throw new std::runtime_error("failed checksums");
		}
	}
...
container::container(psl::string8::view content)
{
	parse(content);
}
...
size_t container::parse(const psl::string8::view& content, std::vector<data>& node_data, std::vector<nodes_t>& reference_nodes, children_t depth)
{
	...
	while (node_begin_n != psl::string8_t::npos)
	{
		...
		if (node_data.size() + 1 == std::numeric_limits<nodes_t>::max())
			throw new max_nodes_reached();
		...
		psl::string8_t tail_name = constants::TAIL_OPEN + psl::string8_t(node_data[index].name()) + constants::TAIL_CLOSE;
		...
		case type::VALUE:
		{
			...
			auto content_end_n = search(content, (literal_value) ? constants::LITERAL_CLOSE : tail_name, content_begin_n);
			...
			node_data[index].reinterpret_as_value() = value_t{ literal_value, std::make_pair(m_Content.size() - content_size_n, content_size_n) };
		}break;
		case type::VALUE_RANGE:
		{
			...
			while(node_content_end_n == psl::string8_t::npos && offset_n != psl::string8_t::npos)
			{
				...
				found_n = content.find_first_not_of(constants::EMPTY_CHARACTERS, found_n + constants::RANGE_CLOSE.size());
			...
			offset_n = content.find_first_not_of(constants::EMPTY_CHARACTERS, content_begin_n) + constants::RANGE_OPEN.size();
			bool bEnd = false;
			...
			while (!bEnd)
			{
				...
				if(sub_content_size_n > 0)
				{
					...
					content_views->emplace_back(literal, std::make_pair(m_Content.size() - sub_content_size_n, sub_content_size_n));
				}
		...
		case type::REFERENCE:
		{
			...
			content_begin_n = content.find_first_not_of(constants::EMPTY_CHARACTERS + constants::REFERENCE, content_begin_n);
		...
		case type::REFERENCE_RANGE:
		{
			...
			std::vector<psl::string8::view>* content_views = new (node_data[index]._data()) std::vector<psl::string8::view>();
			while (!bEnd)
			{
				auto sub_content_begin_n = content.find_first_not_of(constants::EMPTY_CHARACTERS + constants::REFERENCE, offset_n);
				auto sub_content_end_n = psl::string8_t::npos;
		...
		while (next_tail_n <= node_begin_n && next_tail_n != psl::string8_t::npos)
		{
			...
			node_data[view.index].reinterpret_as_collection() = collection_t((nodes_t)(node_data.size() - view.index - 1));
...
void container::parse_references(container& container, std::vector<data>& data, const std::vector<nodes_t>& reference_nodes)
{
...
data& container::internal_get(psl::string8::view view)
{
	return m_NodeData[index_of(view)];
}

handle& container::operator[](nodes_t index)
{
	if((size_t)index >= m_NodeData.size())
		throw new node_not_found(this, index);
	return *m_NodeData[(size_t)index].m_Handle;
}

handle& container::operator[](psl::string8::view view) const
{
...
void data::to_string(const psl::format::settings& settings, psl::string8_t& out) const
{
	...
	out += psl::string8_t(m_Depth * (size_t)(settings.pretty_write), tab) + constants::HEAD_OPEN + node_name + constants::HEAD_CLOSE;
	switch (m_Type)
	{
	case type::COLLECTION:
	{
	...
	case type::REFERENCE_RANGE:
	{
		...
		if (val.size() > 0)
			out.erase(out.size() - constants::RANGE_DIVIDER.size());
		out += constants::RANGE_CLOSE;
	}break;
	default: { std::runtime_error("unknown node type"); };
	}
...
handle& container::add_reference_range(psl::string8::view name, std::vector<std::reference_wrapper<data>> referencing)
{
	...
	for (auto i = 0u; i < referencing.size(); ++i)
		ref_indices.emplace_back(referencing[i].get().m_Handle);
	auto[node_index, depth] = create(name);
...
handle& container::add_reference_range(data& parent, psl::string8::view name, std::vector<std::reference_wrapper<data>> referencing)
{
	reference_range_t ref_indices;
	ref_indices.reserve(referencing.size());
	for (auto i = 0u; i < referencing.size(); ++i)
...
{
	nodes_t node_index = (nodes_t)m_NodeData.size(); // seeing we are adding one afterwards, this value is correct after the 
...
psl::string8_t container::compact_header::build(const container& container)
{
	...
	res.resize(sizeof(psl::string8::char_t) *8 +
				sizeof(size_t) * 2 +
				sizeof(entry) * entries.size() + 
				content_info_size_bytes +
				sizeof(psl::string8::char_t) * names.size() +
				sizeof(psl::string8::char_t) * content.size());


	char* data = res.data();
	memcpy(data, (container.m_Features.encoding == encoding_t::string)?string_identifier.data():bin_identifier.data(), sizeof(psl::string8::char_t) * 8);
...
bool container::compact_header::try_decode(psl::string8::view source, psl::format::container& target)
{
	...
	if(id != string_identifier && id != bin_identifier)
		return false;
...
psl::string8_t container::to_string(std::optional<psl::format::settings> settings) const
{
	...
	{
		size_t reserve = m_Content.size() + m_InternalData.size() + m_InternalData.size() + res.size() + (m_Content.size() + m_InternalData.size()) / 10u;
		for(int i = 0; i < m_NodeData.size(); ++i)
		{
			reserve += m_NodeData[i].m_Depth * (size_t)(setting.pretty_write) + constants::HEAD_OPEN.size() + constants::HEAD_CLOSE.size() +
				constants::TAIL_OPEN.size() + constants::TAIL_CLOSE.size();
			...
			switch(m_NodeData[i].m_Type)
			{
				...
				default: { throw std::runtime_error("malformed node"); };
			}
		...
		{
			res += "VERSION " + std::to_string(m_Features.version.major) + "." + std::to_string(m_Features.version.minor) + '\n';
		...
		if(setting.write_header)
		{
			auto checksum = utility::crc32(psl::string8::view{&res[checksum_content_index], res.size() - checksum_content_index});
...
{
	// todo possibly: cache all known collection indices, this way you can just jump to the next one instead of evalling all nodes
...
std::vector<data>::iterator rfind_last_index_of(std::vector<type> types, std::vector<data>::iterator first, std::vector<data>::iterator last)
{
...
std::vector<data>::iterator find_first_index_of(std::vector<type> types, std::vector<data>::iterator first, std::vector<data>::iterator last)
{
...
void container::move(data& node, nodes_t index)
{
	...
	if (current_index < index)	// forward move
	{
		...
		std::rotate(m_NodeData.begin() + current_index, m_NodeData.begin() + current_index + move_count, m_NodeData.begin() + index);
	}
	else if (current_index > index)	// back move
	{
		std::rotate(m_NodeData.rbegin() + current_index, m_NodeData.rbegin() + current_index + move_count, m_NodeData.rbegin() + index);
	}
}
...
handle& container::find(const data& parent, psl::string8::view name)
{
	...
	if (std::numeric_limits<nodes_t>::max() == index)
		return m_TerminalHandle;
...
container::container(container && other) : m_NodeData(std::move(other.m_NodeData)), m_Content(std::move(other.m_Content)), m_InternalData(std::move(other.m_InternalData)), m_Features(other.m_Features)
{
