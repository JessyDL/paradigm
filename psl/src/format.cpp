...
bool psl::format::container::set_reference(psl::format::data& source, psl::format::data& target) {
	if(source.m_Type != psl::format::type_t::REFERENCE)
		return false;
...
container* psl::format::data::root() const {
	return m_Handle->m_Container;
}
...
data& handle::get() const {
	return m_Container->m_NodeData[m_Index];
};

data* handle::operator->() const {
	return &m_Container->m_NodeData[m_Index];
};
